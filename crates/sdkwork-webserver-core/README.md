...
- Listener `proxyProtocol` is separately opt-in and mandatory per connection when configured. It requires 1..64 unique trusted immediate-source CIDRs, one or both strict `v1`/`v2` versions, a 100..10,000 ms preface timeout, and a 107..4,096 byte total Header ceiling; defaults are both versions, 3 seconds, and 536 bytes. `crc32cPolicy` is one of `ignore`, `validate-if-present`, or `required`, defaults to `ignore`, and requires v2 when checking is enabled. The policy is mutually exclusive with `trustedProxy`, and every field is Restart-only because parsing happens before TLS and HTTP protocol selection.
